...
void main() {
  ...
  group('Splash screen routing', () {
    testWidgets('navigates to onboarding when not onboarded', (tester) async {
      SharedPreferences.setMockInitialValues({});
      final prefs = await SharedPreferences.getInstance();
    ...
    testWidgets('navigates to login when onboarded but not authenticated', (
      tester,
    ) async {
      SharedPreferences.setMockInitialValues({
      ...
      addTearDown(container.dispose);
      final session = container.read(appSessionProvider);
      expect(session.hasSeenOnboarding, true);
      expect(session.isAuthenticated, false);
    });
  });
}
